...
final class GoalInfoViewModel: ObservableObject {
...
//        if let id = self.goal?.id {
//            self.goal = storage.editGoal(id, title: goal?.title, steps: goal?.steps)
//            self.goal = storage.goalEdit(id, title: goal?.title, steps: goal?.steps)
//        }
    }
